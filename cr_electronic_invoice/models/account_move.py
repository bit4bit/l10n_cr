...
class InvoiceLineElectronic(models.Model):
    ...
    total_tax = fields.Float(string="Tax Total", required=False, )

    third_party_id = fields.Many2one(
        "res.partner", string="Tercero otros cargos",)

    tariff_head = fields.Char(
        string="Partida arancelaria para factura de exportación", required=False, )
    ...
    def product_changed(self):
        """Check if the product is non deductible to use a
        non_deductible tax"""
        ...
        # Check for the economic activity in the product or product category or company
        # respectively (already set in the invoice when partner selected)
...
class AccountInvoiceElectronic(models.Model):
    ...
    reference_document_id = fields.Many2one(
        "reference.document", string="Tipo Documento de referencia", required=False)

    payment_methods_id = fields.Many2one(
        "payment.methods", string="Métodos de Pago", required=False)

    invoice_id = fields.Many2one(
        "account.move", string="Documento de referencia", required=False, copy=False)
    ...
    tipo_documento = fields.Selection([('FE', 'Factura Electrónica'),
                                       ('FEE', 'Factura Electrónica de Exportación'),
                                       ('TE', 'Tiquete Electrónico'),
                                       ('NC', 'Nota de Crédito'),
                                       ('ND', 'Nota de Débito'),
                                       ('FEC', 'Factura Electrónica de Compra'),
                                       ('disabled', 'Electronic Documents Disabled')], string="Tipo Comprobante", required=False, default='FE',
        ...
        if self.type in ('in_invoice', 'in_refund'):
            if self.partner_id:
                ...
                if self.partner_id.activity_id:
                    ...
                    raise UserError(
                        _('Partner does not have a default economic activity'))
    ...
    def action_invoice_sent(self):
        ...
        if self.invoice_id.type == 'in_invoice' or self.invoice_id.type == 'in_refund':
            template = self.env.ref(
                'cr_electronic_invoice.email_template_invoice_vendor', raise_if_not_found=False)
        else:
            template = self.env.ref(
    ...
    def _onchange_xml_supplier_approval(self):
        if self.xml_supplier_approval:
            ...
            pretty_xml_string = etree.tostring(factura, pretty_print=True,
    ...
    def load_xml_data(self):
        ...
        analytic_account_id = self.env['ir.config_parameter'].sudo(
        ...
        api_facturae.load_xml_data(
            self, load_lines, account, product, analytic_account)
    ...
    #    else:
    #        raise UserError(
    #            _('You must select the aceptance state: Accepted, Parcial Accepted or Rejected'))
                    ...
                    if inv.state_tributacion == 'procesando':
                        ...
                        if inv.company_id.frm_ws_ambiente != 'disabled' and inv.state_invoice_partner:
                            ...
                            if not inv.xml_comprobante or inv.state_invoice_partner not in ['procesando', 'aceptado']:
                                ...
                                inv.xml_comprobante = base64.encodestring(
                                    xml_firmado)
    ...
    def _check_hacienda_for_invoices(self, max_invoices=10):
        ...
        for i in invoices:
            ...
            if not i.xml_comprobante:
                ...
                _logger.warning(
                    u'E-INV CR - Documento:%s no tiene documento XML.  Estado: %s', i.number_electronic, 'error')
                continue

            if not i.number_electronic or len(i.number_electronic) != 50:
                i.state_tributacion = 'error'
                _logger.warning(
                    u'E-INV CR - Documento:%s no cumple con formato de número electrónico.  Estado: %s', i.number, 'error')
                continue
            ...
            if estado_m_h == 'aceptado':
                ...
                if i.error_count > 10:
                    ...
                    i.state_tributacion = ''
                _logger.error(
                    'MAB - Consulta Hacienda - Invoice not found: %s  -  Estado Hacienda: %s', i.number_electronic, estado_m_h)
    ...
    def _send_invoices_to_hacienda(self, max_invoices=10):  # cron
        ...
        _logger.info(
            'E-INV CR - _send_invoices_to_hacienda - Finalizado Exitosamente')
    ...
    def generate_and_send_invoices(self, invoices):
        ...
        for inv in invoices:
            ...
            if not inv.xml_comprobante or (inv.tipo_documento == 'FEC' and inv.state_tributacion == 'rechazado'):
                ...
                dia = inv.number_electronic[3:5]
                mes = inv.number_electronic[5:7]
                anno = inv.number_electronic[7:9]
                ...
                for inv_line in inv.invoice_line_ids:
                    ...
                    if inv_line.product_id and inv_line.product_id.id == self.env.ref('cr_electronic_invoice.product_iva_devuelto').id:
                        ...
                        if not inv_line.product_uom_id or inv_line.product_uom_id.category_id.name == 'Services':
                            if taxes:
                                if _tax_exoneration:
                                    if _percentage_exoneration < 1:
                                        total_servicio_gravado += (
                                            base_line * (1-_percentage_exoneration))
                ...
                total_mercaderia_exonerado = round(
                    total_mercaderia_exonerado, 5)
                ...
                xml_string_builder = api_facturae.gen_xml_v43(
                    ...
                    total_impuestos, total_descuento, json.dumps(
                ...
                inv.fname_xml_comprobante = inv.tipo_documento + \
                    '_' + inv.number_electronic + '.xml'
            ...
            response_json = api_facturae.send_xml_fe(
                inv, token_m_h, inv.date_issuance, xml_firmado, inv.company_id.frm_ws_ambiente)
    ...
    def get_invoice_sequence(self):
        ...
        if self.partner_id and self.partner_id.vat and not self.partner_id.identification_id:
            raise UserError(
                'Seleccione el tipo de identificación del cliente en su perfil')

        if tipo_documento == 'NC':
            sequence = self.journal_id.refund_sequence_id.number_next
        else:
            sequence = self.journal_id.sequence_id.number_next

        sequence = str(sequence).zfill(10)
    ...
    def action_post(self):
        ...
        for inv in self:
            ...
            if (inv.invoice_id) and not (inv.reference_code_id and inv.reference_document_id):
                raise UserError(
                    'Datos incompletos de referencia para nota de crédito')
                ...
                raise UserError(
                    'Datos incompletos de referencia para nota de crédito no cargada')
            ...
            if self.type == 'in_invoice' and self.partner_id.country_id and \
                ...
                raise UserError(
                    'Las facturas FEC requieren que el proveedor tenga definida la actividad económica')

            # Digital Invoice or ticket
            # Keep original Number Electronic
            if inv.type in ('out_invoice', 'out_refund') and inv.number_electronic:
                pass
            else:
            ...
            if inv.company_id.activity_id.display_name == 'CLINICA, CENTROS MEDICOS, HOSPITALES PRIVADOS Y OTROS' and inv.payment_methods_id.sequence == '02':
                ...
                if iva_devuelto:
                    prod_iva_devuelto = self.env.ref(
                        'cr_electronic_invoice.product_iva_devuelto')
