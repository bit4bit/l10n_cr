...
def get_clave_hacienda(doc, tipo_documento, consecutivo, sucursal_id, terminal_id, situacion='normal'):
    ...
    #inv_consecutivo = is_integer_num(consecutivo)
    ...
    dia = str(doc.invoice_date.day).zfill(2)  # [8:10]#'%02d' % now_cr.day,
    mes = str(doc.invoice_date.month).zfill(2)  # [5:7]#'%02d' % now_cr.month,
    anno = str(doc.invoice_date.year)[2:]  # str(now_cr.year)[2:4],
...
def gen_xml_mr_43(clave, cedula_emisor, fecha_emision, id_mensaje,
    ...
    if monto_total_impuesto_acreditar:
        sb.Append('<MontoTotalImpuestoAcreditar>' +
                  str(monto_total_impuesto_acreditar) + '</MontoTotalImpuestoAcreditar>')
...
def gen_xml_v43(inv, sale_conditions, total_servicio_gravado,
    ...
    if inv._name == 'pos.order':
        ...
        plazo_credito = str(
            inv.invoice_payment_term_id and inv.invoice_payment_term_id.line_ids[0].days or 0)
    ...
    sb.Append('<NombreComercial>' +
              escape(str(issuing_company.commercial_name or 'NA')) + '</NombreComercial>')
    sb.Append('<Ubicacion>')
    ...
    if issuing_company.neighborhood_id and issuing_company.neighborhood_id.code:
        sb.Append(
            '<Barrio>' + str(issuing_company.neighborhood_id.code or '00') + '</Barrio>')

    sb.Append('<OtrasSenas>' +
              escape(str(issuing_company.street or 'NA')) + '</OtrasSenas>')
    sb.Append('</Ubicacion>')

    if issuing_company.phone:
        phone = phonenumbers.parse(
            issuing_company.phone, (issuing_company.country_id.code or 'CR'))
        sb.Append('<Telefono>')
        sb.Append('<CodigoPais>' + str(phone.country_code) + '</CodigoPais>')
        sb.Append('<NumTelefono>' +
                  str(phone.national_number) + '</NumTelefono>')
        sb.Append('</Telefono>')

    sb.Append('<CorreoElectronico>' +
              str(issuing_company.email) + '</CorreoElectronico>')
    sb.Append('</Emisor>')
    ...
    if inv.tipo_documento == 'TE' or (inv.tipo_documento == 'NC' and not receiver_company.vat):
        ...
        if receiver_company.name:
            sb.Append('<Receptor>')
            sb.Append(
                '<Nombre>' + escape(str(receiver_company.name[:99])) + '</Nombre>')

            if inv.tipo_documento == 'FEE':
                if receiver_company.vat:
                    sb.Append('<IdentificacionExtranjero>' +
                              receiver_company.vat + '</IdentificacionExtranjero>')
            else:
                sb.Append('<Identificacion>')
            ...
            if inv.tipo_documento != 'FEE':
                if receiver_company.state_id and receiver_company.county_id and receiver_company.district_id and receiver_company.neighborhood_id:
                    sb.Append('<Ubicacion>')
                    sb.Append(
                        '<Provincia>' + str(receiver_company.state_id.code or '') + '</Provincia>')
                    sb.Append(
                        '<Canton>' + str(receiver_company.county_id.code or '') + '</Canton>')
                    sb.Append(
                        '<Distrito>' + str(receiver_company.district_id.code or '') + '</Distrito>')

                    if receiver_company.neighborhood_id and receiver_company.neighborhood_id.code:
                        sb.Append(
                            '<Barrio>' + str(receiver_company.neighborhood_id.code or '00') + '</Barrio>')

                    sb.Append(
                        '<OtrasSenas>' + escape(str(receiver_company.street or 'NA')) + '</OtrasSenas>')
                    sb.Append('</Ubicacion>')

                if receiver_company.phone:
                    try:
                        phone = phonenumbers.parse(
                            receiver_company.phone, (receiver_company.country_id.code or 'CR'))
                        sb.Append('<Telefono>')
                        sb.Append('<CodigoPais>' +
                                  str(phone.country_code) + '</CodigoPais>')
                        sb.Append('<NumTelefono>' +
                                  str(phone.national_number) + '</NumTelefono>')
                        sb.Append('</Telefono>')
                    except:
    ...
    for payment_method_counter in range(payment_method_length):
        sb.Append('<MedioPago>' +
                  payment_methods_id[payment_method_counter] + '</MedioPago>')
    ...
    if lines != "{}":
        ...
        for (k, v) in response_json.items():
            ...
            if inv.tipo_documento == 'FEE' and v.get('partidaArancelaria'):
                sb.Append('<PartidaArancelaria>' +
                          str(v['partidaArancelaria']) + '</PartidaArancelaria>')
                    ...
                    if tax_code.isdigit():
                        ...
                        if inv.tipo_documento != 'FEE':
                            if b.get('exoneracion'):
                                ...
                                sb.Append(
                                    '<TipoDocumento>' + receiver_company.type_exoneration.code + '</TipoDocumento>')
                                sb.Append(
                                    '<NumeroDocumento>' + receiver_company.exoneration_number + '</NumeroDocumento>')
                                sb.Append(
                                    '<NombreInstitucion>' + receiver_company.institution_name + '</NombreInstitucion>')
                                sb.Append('<FechaEmision>' + str(receiver_company.date_issue) +
                                          'T00:00:00-06:00' + '</FechaEmision>')
    ...
    if otrosCargos:
        ...
        for otro_cargo in otrosCargos:
            sb.Append('<TipoDocumento>' +
                      str(otrosCargos[otro_cargo]['TipoDocumento']) + '</TipoDocumento>')

            if otrosCargos[otro_cargo].get('NumeroIdentidadTercero'):
                sb.Append('<NumeroIdentidadTercero>' + str(
                    otrosCargos[otro_cargo]['NumeroIdentidadTercero']) + '</NumeroIdentidadTercero>')

            if otrosCargos[otro_cargo].get('NombreTercero'):
                sb.Append(
                    '<NombreTercero>' + str(otrosCargos[otro_cargo]['NombreTercero']) + '</NombreTercero>')

            sb.Append('<Detalle>' +
                      str(otrosCargos[otro_cargo]['Detalle']) + '</Detalle>')

            if otrosCargos[otro_cargo].get('Porcentaje'):
                sb.Append(
                    '<Porcentaje>' + str(otrosCargos[otro_cargo]['Porcentaje']) + '</Porcentaje>')

            sb.Append(
                '<MontoCargo>' + str(otrosCargos[otro_cargo]['MontoCargo']) + '</MontoCargo>')
        sb.Append('</OtrosCargos>')
    ...
    sb.Append('<TotalGravado>' + str(round(total_servicio_gravado +
                                           total_mercaderia_gravado, 5)) + '</TotalGravado>')
    sb.Append('<TotalExento>' + str(round(total_servicio_exento +
                                          total_mercaderia_exento, 5)) + '</TotalExento>')

    if inv.tipo_documento != 'FEE':
        sb.Append('<TotalExonerado>' + str(round(totalServExonerado +
                                                 totalMercExonerada, 5)) + '</TotalExonerado>')
    ...
    if total_iva_devuelto:
        sb.Append('<TotalIVADevuelto>' +
                  str(round(total_iva_devuelto, 5)) + '</TotalIVADevuelto>')

    sb.Append('<TotalOtrosCargos>' +
              str(totalOtrosCargos) + '</TotalOtrosCargos>')
    sb.Append('<TotalComprobante>' + str(round(base_total + total_impuestos +
                                               totalOtrosCargos - total_iva_devuelto, 5)) + '</TotalComprobante>')
    sb.Append('</ResumenFactura>')
    ...
    if tipo_documento_referencia and numero_documento_referencia and fecha_emision_referencia:
        ...
        sb.Append('<FechaEmision>' +
                  fecha_emision_referencia + '</FechaEmision>')
...
def get_economic_activities(company):
    ...
    if 200 <= response.status_code <= 299:
        _logger.debug('MAB - get_economic_activities response: %s' %
                      (response.json()))
        response_json = {
            'status': 200,
...
def load_xml_data(invoice, load_lines, account_id, product_id=False, analytic_account_id=False):
    ...
    try:
        invoice_xml = etree.fromstring(
            base64.b64decode(invoice.xml_supplier_approval))
        document_type = re.search(
            'FacturaElectronica|NotaCreditoElectronica|NotaDebitoElectronica|TiqueteElectronico', invoice_xml.tag).group(0)

        if document_type == 'TiqueteElectronico':
            raise UserError(
                _("This is a TICKET only invoices are valid for taxes"))
    ...
    invoice.reference = invoice_xml.xpath(
    ...
    activity_node = invoice_xml.xpath(
        "inv:CodigoActividad", namespaces=namespaces)
    activity_id = False
    activity = False
    if activity_node:
        activity = invoice.env['economic.activity'].with_context(
            active_test=False).search([('code', '=', activity_node[0].text)], limit=1)
    ...
    currency_node = invoice_xml.xpath(
        "inv:ResumenFactura/inv:CodigoTipoMoneda/inv:CodigoMoneda", namespaces=namespaces)
    if currency_node:
        invoice.currency_id = invoice.env['res.currency'].search(
            [('name', '=', currency_node[0].text)], limit=1).id
    else:
        invoice.currency_id = invoice.env['res.currency'].search(
            [('name', '=', 'CRC')], limit=1).id
    ...
    if partner:
        ...
        raise UserError(
            _('The provider in the invoice does not exists. Please review it.'))
    ...
    payment_method_node = invoice_xml.xpath(
        "inv:MedioPago", namespaces=namespaces)
    ...
    if load_lines:
        lines = invoice_xml.xpath(
            "inv:DetalleServicio/inv:LineaDetalle", namespaces=namespaces)
        new_lines = []
        for line in lines:
            product_uom = invoice.env['uom.uom'].search(
                [('code', '=', line.xpath("inv:UnidadMedida",
                                          namespaces=namespaces)[0].text)],
                limit=1).id
            total_amount = float(line.xpath(
                "inv:MontoTotal", namespaces=namespaces)[0].text)

            discount_percentage = 0.0
            discount_note = None

            if total_amount > 0:
                discount_node = line.xpath(
                    "inv:Descuento", namespaces=namespaces)
                if discount_node:
                    discount_amount_node = discount_node[0].xpath(
                        "inv:MontoDescuento", namespaces=namespaces)[0]
                    discount_amount = float(discount_amount_node.text or '0.0')
                    discount_percentage = discount_amount / total_amount * 100
                    discount_note = discount_node[0].xpath(
                        "inv:NaturalezaDescuento", namespaces=namespaces)[0].text
                else:
                    discount_amount_node = line.xpath(
                        "inv:MontoDescuento", namespaces=namespaces)
                    if discount_amount_node:
                        discount_amount = float(
                            discount_amount_node[0].text or '0.0')
                        discount_percentage = discount_amount / total_amount * 100
            ...
            for tax_node in tax_nodes:
                ...
                tax_amount = float(tax_node.xpath(
                    "inv:Tarifa", namespaces=namespaces)[0].text)
                ...
                if tax:
                    total_tax += float(tax_node.xpath("inv:Monto",
                                                      namespaces=namespaces)[0].text)

                    exonerations = tax_node.xpath(
                        "inv:Exoneracion", namespaces=namespaces)
                    if exonerations:
                        for exoneration_node in exonerations:
                            exoneration_percentage = float(exoneration_node.xpath(
                                "inv:PorcentajeExoneracion", namespaces=namespaces)[0].text)
                            tax = invoice.env['account.tax'].search(
                    ...
                    if product_id and product_id.non_tax_deductible:
                        raise UserError(
                            _('Tax code %s and percentage %s as non-tax deductible is not registered in the system' % (tax_code, tax_amount)))
                    else:
